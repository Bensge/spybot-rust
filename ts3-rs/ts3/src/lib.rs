...
impl Decode<String> for String {
    fn decode(buf: &[u8]) -> Result<String, BoxError> {
        ...
        // Create a peekable iterator to iterate over all chars, appending all chars
                ...
                '\\' => {
                    match iter.peek() {
                        Some(c) => match c {
                            ...
                            's' => result.push(' '),
                            'p' => result.push('|'),
                            'a' => result.push(7u8 as char),
                            'b' => result.push(8u8 as char),
                            'f' => result.push(12u8 as char),
                            'n' => result.push(10u8 as char),
                            'r' => result.push(13u8 as char),
                            't' => result.push(9u8 as char),
                            'v' => result.push(11u8 as char),
                            _ => return Err(Box::new(DecodeError::UnexpectedChar(*c))),
                        },
