    ...
    pub async fn send<T: Decode<T>>(&self, cmd: String) -> Result<T> {
            ...
            .await
        {
            Ok(_) => {
                ...
                Ok(T::decode(&resp.unwrap()?).unwrap())
            }
...
impl Client {
    ...
    pub async fn clientlist(&self) -> ts3::client::Result<Vec<ClientInfo>> {
        self.send("clientlist -uid".to_owned()).await
    }
}
...
/// Client metadata returned from clientlist
#[derive(Debug, Decode, Default)]
